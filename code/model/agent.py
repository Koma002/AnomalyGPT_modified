...
class DeepSpeedAgent:
    ...
    def __init__(self, model, args):
        ...
        if args['if_load_delta']:
            delta_ckpt = torch.load(args['delta_ckpt_path'], map_location=torch.device('cpu'))
            self.model.load_state_dict(delta_ckpt, strict=False)
            print("[!] delta checkpoint initialized")
        ...
        for name, param in self.model.prompt_learner.named_parameters():
            param.requires_grad = True
        #
        for name, param in self.model.gemma_proj.named_parameters():
            param.requires_grad = True



        # load config parameters of deepspeed
        with open(self.args['ds_config_path'], 'r') as f:
    ...
    def save_model_gemma(self, path, current_step):
        # only save trainable model parameters
        checkpoint = OrderedDict()
        for k, v in self.ds_engine.module.named_parameters():
            if v.requires_grad:
        ...
        torch.save(checkpoint, f'{path}/pytorch_model.pt')
        gemma_path = path+'gemma_weight/'
        # save tokenizer
        self.model.tokenizer.save_pretrained(gemma_path)
        # save configuration
        self.model.gemma_model.save_pretrained(gemma_path)
        print(f'[!] save model into {gemma_path}')

    def save_model_gemma_2(self, path, current_step):
        # only save trainable model parameters
        checkpoint = OrderedDict()
        for k, v in self.ds_engine.module.named_parameters():
            if v.requires_grad:
